...
interface StatCardProps {
    ...
    value: number | string;
    label: string;
    color?: 'gray' | 'blue' | 'green' | 'yellow' | 'red' | 'purple' | 'pink' | 'cyan' | 'orange';
    onClick?: () => void;
    className?: string;
    isSelected?: boolean;
}
      ...
      `}
            >
                {typeof icon === 'string' ? (
                    ...
                    <div
                        className={`
            w-6 h-6 xl:w-6 xl:h-6 lg:text-5 md:text-4.5 sm:text-4
          ...
          `}
            ...
            <div className="flex flex-col flex-1 min-w-0">
                <div className={`text-xl xl:text-xl lg:text-lg md:text-base sm:text-sm font-bold leading-none tracking-tight mb-1 ${isSelected
                    ? variant.iconColor.replace('text-', 'text-').replace('-500', '-700').replace('-600', '-800')
