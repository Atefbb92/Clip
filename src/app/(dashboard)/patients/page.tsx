...
const PatientsPage = () => {
  ...
  const [sortBy, setSortBy] = useState<'patient' | 'status' | 'category' | 'date' | 'validationDate'>('date')
  const [sortDir, setSortDir] = useState<'asc' | 'desc'>('desc')

  const toggleSort = (key: 'patient' | 'status' | 'category' | 'date' | 'validationDate') => {
    if (sortBy === key) {
  ...
  const getSortIcon = (key: 'patient' | 'status' | 'category' | 'date' | 'validationDate') => {
  ...
  const sortedPatients = React.useMemo(() => {
    ...
    arr.sort((a, b) => {
      ...
      if (sortBy === 'category') {
        const ac = categoryOf(a.age).toLowerCase()
        const bc = categoryOf(b.age).toLowerCase()
  ...
  const getStatusBadge = (status: number) => {
    const statusConfig = {
      ...
      2: { label: t('status.en-attente'), color: 'bg-yellow-100 text-yellow-800' },
  ...
  const formatDate = (date: Timestamp | Date | null | undefined) => {
    if (!date) return 'N/A'
    try {
      const d = date instanceof Date ? date : date instanceof Timestamp ? date.toDate() : null
  ...
  return (
    <div className="min-h-screen p-8">
      ...
      {/* Status Cards */}
      <StatusCard selectedStatus={statusFilter} onStatusSelect={handleStatusSelect} />
          ...
          <nav className="flex space-x-8 px-6">
            ...
            >
              ...
              <span className="bg-red-100 text-red-800 text-xs font-medium px-2 py-0.5 rounded-full">
                {
                  patients.filter(
                    (patient) => [0, 2, 3].includes(patient.status) && patient.archived !== 1
                  ).length
                }
              </span>
            </button>
            ...
            >
              {t('patients.tabs.archived')} ({patients.filter((p) => p.archived === 1).length})
            </button>
          </nav>
        </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {sortedPatients.length === 0 ? (
                ...
                sortedPatients.map((patient: Patient) => {
                  const category = categoryOf(patient.age)
                  return (
                    <tr
                      key={patient.id}
                    ...
                    >
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <Avatar className="h-10 w-10 mr-4">
                            <AvatarImage src={patient.images?.img2} />
                          ...
                          <div>
                            ...
                            <div className="text-[9px] text-gray-500">{patient.id}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {patient.validationDate ? formatDate(patient.validationDate) : '-'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {(() => {
                                  ...
                                  <Tooltip>
                                    ...
                                    <TooltipContent className="bg-gray-800 text-white">
                                      <ul className="text-sm space-y-1">
                                        {remainingConditions.map((condition, index) => (
